// src/components/RolFormModal.jsx
import React, { useState, useEffect } from 'react';
import { FaSave, FaTimes } from 'react-icons/fa';

const RolFormModal = ({ isOpen, onClose, onSubmit, initialData }) => {
  const [nombreRol, setNombreRol] = useState('');
  const [formError, setFormError] = useState('');

  useEffect(() => {
    if (isOpen) {
      setNombreRol(initialData ? initialData.NombreRol : '');
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg relative p-6 sm:p-8">
        ...
        <div className="flex justify-between items-center mb-6 border-b pb-4">
          <h2 className="text-3xl font-extrabold text-gray-800 flex-1 text-center">
            {initialData ? 'Editar Rol' : 'Agregar Nuevo Rol'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-5"> 
            <label htmlFor="nombreRol" className="block text-gray-700 text-sm font-semibold mb-2">
